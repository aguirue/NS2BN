...
public class GeneticSearch 
	...
	public class BayesNetRepresentation {
		...
		public void randomInit() {
			do {
				...
				for (int i = 0; i < m_nNodes; i++) {
					int iPos;
					do {
						iPos = new Random().nextInt(m_nNodes * m_nNodes);
					} while (isSquare(iPos));
					m_bits[iPos] = true;
				}
			} while (hasCycles());
		 ...
		 */
		GeneticSearch.BayesNetRepresentation copy() {
			GeneticSearch.BayesNetRepresentation b = new GeneticSearch.BayesNetRepresentation(m_nNodes);
		 ...
		 */
		void crossOver(GeneticSearch.BayesNetRepresentation other) {
	...
	protected void search(BayesNet bayesNet, Instances instances) throws Exception {
		...
		GeneticSearch.BayesNetRepresentation  [] population = new GeneticSearch.BayesNetRepresentation [getPopulationSize()];
        for (int i = 0; i < getPopulationSize(); i++) {
        	population[i] = new GeneticSearch.BayesNetRepresentation (instances.numAttributes());
        ...
        for (int iRun = 0; iRun < m_nRuns; iRun++) {
        	// create descendants
			GeneticSearch.BayesNetRepresentation  [] descendantPopulation = new GeneticSearch.BayesNetRepresentation  [getDescendantPopulationSize()];
